...
void solveTask(struct Student *students, int totalStudents) {
    ...
    fprintf(file, "-----------------------------------------------------------------------------------\n");
    for (int i = 0; i < totalStudents; i++) {
        if ((students[i].physicsGrade == 4 || students[i].physicsGrade == 5) && ( students[i].mathGrade > SUCCESS_GRADE &&  students[i].computerScienceGrade > SUCCESS_GRADE)) {

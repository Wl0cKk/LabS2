...
int task_11(Node* node) {
    ...
    if (node->item->name[0] == LETTER) {
        count++;
    }

    count += task_11(node->left);
    count += task_11(node->right);

    return count;
}

Item items[] = {{"Dfg", 1000}, {"ASD", 9000}, {"Qwer", 2000}, {"LKJMN", 8000}, {"Azxcv", 3000}, {"SAKD", 5000}, {"EUIWD", 4000}, {"LPOIK", 2222}};
...
int main() {
    ...
    Item* newItem = create_item("Abcd", 4222);
    ...
    printf("\nNumber of items in the Tree starting with letter '%c': %d", LETTER, task_11(root));

...
void solveTask(struct Student *students, int totalStudents, int targetGroup) {    
    ...
    for (int i = 0; i < totalStudents; i++) {
        bool progress = ( students[i].mathGrade < INADMISSIBLE_GRADE || 
          students[i].physicsGrade < INADMISSIBLE_GRADE || 
          students[i].computerScienceGrade < INADMISSIBLE_GRADE
        );
        if (students[i].groupNumber == targetGroup && progress) { 
            char *res = "---\nLast name: %s, ![student has academic problems]!\n---\n";

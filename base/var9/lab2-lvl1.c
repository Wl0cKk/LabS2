...
void solveTask(struct Student *students, int totalStudents, int targetGroup) {
    float commonGPA = 0.00;
    
    FILE *file = fopen("base.txt", "a");
    fprintf(file, "\n-----------------------------------------------------------------------------------\n");
    for (int i = 0; i < totalStudents; i++) { 
        commonGPA += students[i].averageGrade;
        
    }
    
    commonGPA /= totalStudents;
    printf("Total GPA: %f\n", commonGPA); fprintf(file, "Total GPA: %f\n", commonGPA);
    
    for (int i = 0; i < totalStudents; i++) {
        if (students[i].groupNumber == targetGroup && students[i].averageGrade > commonGPA) { 
            char *res = "---\nLast name: %s, GPA: %.1f\n---\n";

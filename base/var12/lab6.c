...
void find_closest_key(Node* node, float average, int* closestKey, float* minDifference);
...
float calculate_average(Node* node, int* totalSum, int* nodesCount){
    ...
    *totalSum += node->item->uid;
    (*nodesCount)++;
    float rightSum = calculate_average(node->right, totalSum, nodesCount);

    return leftSum + rightSum + node->item->uid;
}

void task_12(Node* root) {
    int totalSum = 0;
    int nodesCount = 0;
    float average = calculate_average(root, &totalSum, &nodesCount) / nodesCount;

    int closestKey = root->item->uid;
    float minDifference = fabsf(average - closestKey);
    
    find_closest_key(root, average, &closestKey, &minDifference);

    printf("\nAverage of all keys in the tree: %.2f\n", average);
    printf("Closest key to the average: %d and value is %s\n", closestKey, search_by_uid(root, closestKey)->name);
}
...
void find_closest_key(Node* node, float average, int* closestKey, float* minDifference) {
    ...
    float currentDifference = fabsf(average - node->item->uid);
    
    if (currentDifference < *minDifference) {
        *minDifference = currentDifference;
        *closestKey = node->item->uid;
    }

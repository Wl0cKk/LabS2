...
void solveTask(struct Student *students, int totalStudents) {
    ...
    fprintf(file, "-----------------------------------------------------------------------------------\n");
    for (int i = 0; i < totalStudents; i++) {
        if ((students[i].physicsGrade == 7) || (students[i].physicsGrade == 8)) {
            char *res = "Last name: %s, Group: %d, physics grade: %d GPA: %.2f\n";
            printf(res, students[i].lastname, students[i].groupNumber, students[i].physicsGrade, students[i].averageGrade);
            fprintf(file, res, students[i].lastname, students[i].groupNumber, students[i].physicsGrade, students[i].averageGrade);
        }
        if ((students[i].mathGrade == 9) || (students[i].mathGrade == 10)) {
            // Студенты еще не изучали высшую математику, у них оценки только по математике
            char *res = "Last name: %s, Group: %d, math grade: %d GPA: %.2f\n";
            printf(res, students[i].lastname, students[i].groupNumber, students[i].mathGrade, students[i].averageGrade);
            fprintf(file, res, students[i].lastname, students[i].groupNumber, students[i].mathGrade, students[i].averageGrade);
        }
    }

...
int task_2(Node* node) {
    ...
    int left_leaves = task_2(node->left);
    int right_leaves = task_2(node->right);
...
int main() {
    ...
    printf("\nNumber of leaves from which there are no links to other nodes in the tree: %d\n", task_2(root));
